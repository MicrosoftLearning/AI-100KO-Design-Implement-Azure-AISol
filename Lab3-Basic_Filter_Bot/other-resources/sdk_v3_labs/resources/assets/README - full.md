# lab02.3-luis_and_search - LUIS 및 Azure Search를 사용하여 지능형 응용 프로그램 개발

이 실습 랩에서는 Microsoft Bot Framework, Azure Search 및 여러 가지 Cognitive Services를 사용하여 종단 간 지능형 봇을 만드는 방법을 안내합니다. 
...
이 워크샵에서는 사진을 가져오고, Cognitive Services를 사용하여 이미지에서 개체와 사람을 찾고, 그 사람들의 감정을 파악하고, 이러한 모든 데이터를 NoSQL Store(DocumentDB)에 저장하는 종단 간 시나리오를 빌드합니다. NoSQL Store를 사용하여 Azure Search 인덱스를 채운 다음, 쉬운 대상 쿼리가 가능하도록 LUIS를 사용해 Bot Framework 봇을 빌드합니다.
...
3. 열린 후에는 Windows 10 App Development(UWP)용 SDK를 설치하라는 메시지가 표시됩니다. 프롬프트에 따라 설치합니다. 메시지가 표시되지 않을 경우 TestApp을 마우스 오른쪽 단추로 클릭하고 "프로젝트 다시 로드"를 선택하면 메시지가 표시됩니다.
...
소문자, 숫자, 대시만 사용해야 한다는 제약 조건에 따라 원하는 ID를 선택합니다. Mongo가 아닌 Document DB SDK를 사용할 것이므로 NoSQL API로 Document DB를 선택합니다. 이전 단계에 사용한 것과 동일한 리소스 그룹 및 동일한 위치를 사용하며, 지속적으로 추적하고 쉽게 돌아올 수 있도록 _대시보드에 고정_ 을 선택한 다음 만들기를 누릅니다.
...
이 워크샵에서는 모든 Cognitive Services API가 아니라 Azure Search와 LUIS에 초점을 둡니다. 그러나 Cognitive Services에 대한 실습을 더 해보고 싶은 경우 TODO: Add reference lab link-Lab 1을 통해 다양한 Cognitive Services를 사용하여 지능형 키오스크를 빌드하는 것이 좋습니다. 
...
Azure Portal 내에서 **새로 만들기->웹 + 모바일->Azure Search** 를 클릭합니다.
...
최종 인덱스 구성 단계에서는 자동 완성에 사용될 필드를 설정하여 사용자가 단어의 일부를 입력하면 Azure Search가 이러한 필드에서 가장 일치하는 항목을 찾을 수 있도록 합니다.
