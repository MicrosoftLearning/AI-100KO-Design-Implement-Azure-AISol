...
## 랩 6.2: 포털에서 LUIS 서비스 만들기(선택 사항)

> 참고: LUIS는 랩에 사용할 수 있는 "시작 키"를 제공하므로 포털에서 LUIS 서비스를 만드는 것은 선택 사항입니다. 그러나 포털에서 무료 또는 유료 서비스를 만드는 방법을 확인하려면 아래 단계를 따를 수 있습니다.

포털에서 **리소스 만들기** 를 누른 후 검색 상자에 **LUIS** 를 입력하고 **Language Understanding** 을 선택합니다.

다음으로, 만들 API 끝점에 대한 몇 가지 세부 정보를 작성하고 원하는 API와 끝점을 사용할 위치 그리고 요금제를 선택합니다. 사용 가능한 가까운 위치에 배치하십시오. 이 랩에는 무료 계층으로 충분합니다.

새 API 구독을 만든 후에는 블레이드의 해당 섹션에서 키를 가져와 키 목록에 추가할 수 있습니다.

![Cognitive API 키](../images//CreateBot2.png))

## 랩 6.3: LUIS를 사용하여 응용 프로그램에 인텔리전스 추가
...
> **경고**: Azure 서비스는 IE를 기본 브라우저로 사용하지만 LUIS에는 사용하지 않는 것이 좋습니다. 모든 랩에서 Firefox를 사용할 수 있습니다. 아니면 [Microsoft Edge](https://www.microsoft.com/ko-kr/download/details.aspx?id=48126) 또는 [Google Chrome](https://www.google.com/intl/en/chrome/)을 다운로드할 수 있습니다.

[https://www.luis.ai](https://www.luis.ai)(**유럽 또는 오스트레일리아에 있는 경우 제외**)로 이동하여 Microsoft 계정을 사용하여 로그인합니다.  이전 섹션에서 LUIS 키를 만드는 데 사용한 계정과 동일해야 합니다.  LUIS 응용 프로그램 목록으로 리디렉션됩니다.  봇을 지원하는 새로운 LUIS 앱을 만들 것입니다.

> * **유럽** 지역에서 키를 만든 경우 [https://eu.luis.ai/](https://eu.luis.ai/)에서 응용 프로그램을 만들어야 합니다. **오스트레일리아** 지역에서 키를 만든 경우 [https://au.luis.ai/](https://au.luis.ai/)에서 응용 프로그램을 만들어야 합니다. LUIS 게시 지역에 대한 자세한 내용은 [여기](https://docs.microsoft.com/ko-kr/azure/cognitive-services/luis/luis-reference-regions)에서 확인할 수 있습니다.
>
...
기본 페이지에서 "LUIS 앱 만들기" 단추를 클릭하고, 다음 페이지에서 국가를 선택하고, 라이선스 계약에 동의하고, 다음 페이지에서 "새 앱 만들기" 단추를 클릭합니다.  이름을 지정하고(예로 "PictureBotLuisModel"을 선택함) 문화권을 "영어"로 설정합니다.  선택적으로 설명을 제공할 수 있습니다. 그런 다음 "완료"를 클릭합니다.
...
> "None"이라는 의도가 있습니다.  의도에 매핑되지 않는 임의의 발화는 "None"에 매핑될 수 있습니다.
...
이러한 각 작업을 요청하는 사용자의 의도를 만들어 보겠습니다.  "새 의도 만들기" 단추를 클릭합니다.

첫 번째 의도의 이름을 "Greeting"으로 지정하고 "완료"를 클릭합니다.  그런 다음 사용자가 봇과 인사할 때 말할 수 있는 몇 가지 예를 제시하고 매번 "Enter" 키를 누릅니다.
...
왼쪽 열에서 "엔터티"를 클릭한 다음 "새 엔터티 만들기"를 클릭합니다.  엔터티 이름을 "패싯"으로 지정하고 엔터티 유형을 ["단순"](https://docs.microsoft.com/ko-kr/azure/cognitive-services/LUIS/luis-concept-entity-types)으로 지정합니다.  그런 다음 "완료"를 클릭합니다.

![패싯 엔터티 추가](../images//LuisCreateEntity.png)

그런 다음 왼쪽 사이드바에서 "의도"를 클릭한 다음 "새 의도 만들기" 단추를 클릭합니다.  "SearchPics"의 의도 이름을 지정한 다음 "완료"를 클릭합니다.
...
+ 음식 사진을 찾아 주세요.
+ 노는 소년들의 사진을 검색해 주세요.
+ 비즈니스 우먼의 사진을 제공해 주세요.
+ 해변 사진을 보여 주세요.
+ 개 사진을 찾고 있어요.
+ 실내에 있는 남자의 사진을 검색해 주세요.
+ 안경을 쓴 남자의 사진을 보여 주세요.
+ 슬픈 소년들의 사진을 보고 싶어요.
+ 행복한 아기 사진을 보여 주세요.

몇 가지 발화를 확보했으면 LUIS에게 "패싯" 엔터티로 **검색 항목** 을 선택하는 방법을 가르쳐야 합니다. "패싯" 엔터티가 선택하는 것이 검색됩니다. 단어를 마우스로 가리키고 클릭한 다음(또는 연속된 단어를 클릭하여 단어 그룹을 선택) "패싯" 엔터티를 선택합니다.
...
다음으로 왼쪽 사이드바에서 "의도"를 클릭하고 다음 두 가지 의도를 추가합니다.
...
마지막으로 일부 샘플 발화를 "None" 의도에 추가해야 합니다. 이렇게 하면 기능이 응용 프로그램의 범위를 벗어날 경우 LUIS 레이블을 지정할 수 있습니다. "피자가 먹고 싶어요", "비디오를 검색해 주세요" 등을 추가해 보십시오. None 의도 내에 앱 발화의 약 10~15%가 있어야 합니다.

## 랩 6.4: LUIS 모델 학습

이제 모델을 학습시킬 수 있습니다.

오른쪽 상단에서 "학습"을 클릭합니다.  제공한 학습 데이터와 함께 발화--> 의도 매핑을 수행하는 모델이 빌드됩니다. 학습이 항상 즉각적으로 이루어지는 것은 아닙니다. 경우에 따라 큐에서 대기하고 몇 분 정도 걸릴 수 있습니다.

다음으로 상단 표시줄에서 "관리"를 클릭합니다. 창 왼쪽에 몇 가지 옵션(응용 프로그램 정보, 키 및 끝점, 게시 설정, 버전, 공동 작업자)이 있습니다. 다양한 게시 옵션에 대한 자세한 내용은 [여기](https://docs.microsoft.com/ko-kr/azure/cognitive-services/LUIS/PublishApp)에서 확인할 수 있습니다.

포털에서 LUIS 서비스를 만드는 선택적 단계를 완료한 경우 이전에 설정한 끝점 키를 선택 또는 추가하거나("키 및 끝점" 탭의 페이지 하단에 있는 "키 추가" 단추 클릭) 링크를 따라 Azure 계정에서 키를 추가합니다.
(테스트 및 학습 목적을 위해 "Starter_Key"를 사용할 수도 있으므로 LUIS 서비스를 만드는 것은 선택 사항입니다.) 끝점 슬롯을 "프로덕션"으로 유지할 수 있습니다.  그런 다음 상단 표시줄에서 "게시"를 클릭합니다. "프로덕션" 또는 "스테이징" 끝점에 게시할 수 있는 옵션이 있습니다. "프로덕션"을 선택하고 [두 끝점을 사용하는 이유](https://docs.microsoft.com/ko-kr/azure/cognitive-services/luis/luis-concept-version)를 읽어보십시오. 마지막으로 "게시"를 클릭합니다.

![LUIS 앱 게시](../images//LuisPublish.png)

게시하면 LUIS 모델을 호출하는 끝점이 만들어지고  URL이 표시되며, 관련 내용은 이후 랩에서 설명합니다. 지금은 끝점 URL을 복사하여 키 목록에 추가할 수 있습니다.

오른쪽 상단에서 "테스트"를 클릭합니다. 몇 가지 발화를 입력하고 반환된 의도를 확인합니다. 현재 또는 향후 랩에서 이 작업을 수행할 수 있으므로 [대화형 테스트](https://docs.microsoft.com/ko-kr/azure/cognitive-services/LUIS/Train-Test#interactive-testing) 및 [끝점 발화 검토](https://docs.microsoft.com/ko-kr/azure/cognitive-services/LUIS/luis-how-to-review-endoint-utt)를 익히십시오.
